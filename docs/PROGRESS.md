...
- **Focus mode (§19–23)** — Phase 6a (single sessions) done: session plans, interrupts/resume with `seconds_away`, completion/abandonment, expiry, XP award. **Phase 6b (Pomodoro breaks, `on_break` status, study/break rounds, focus analytics)** not built.
...
- **Phase 6b — Pomodoro & focus analytics (§22–23):** study/break rounds, `on_break` status, break timers, and session analytics over the recorded focus data (started_at, active/break seconds, tab switches, topic, concepts, questions, completion). Model columns (`study_minutes`, `break_minutes`, `rounds`, `current_round`, `break_seconds`) already exist.
...
`docs/API_CONTRACT.md` documents **15** of the 24 endpoints: subjects, subject topics, topic notes, create assessment, submit assessment, flashcards deck, flashcard review, record activity, gamification profile, achievements, start focus, get focus state, interrupt, resume, complete.
...
| `focus_sessions` / `focus_interruptions` | `models/focus_session.py` | Phase 6a focus sessions; Pomodoro columns included for Phase 6b |
...
- Tests: `python -m pytest` from `backend/` — **72 passed** after Phase 6a (13 focus tests added) (deprecation warnings only, no failures).
...
6. **Stub files look done but are empty** — `progress.py`, `feedback.py`, `parent.py` routes; `feedback_service.py`; `feedback.py` model; `ingest_curriculum.py`; `core/security.py`. Easy to mistake for implemented (`focus.py`, `focus_service.py`, `focus_session.py` are now implemented in Phase 6a).
7. Git history shows 7 commits (`initial setup` → `phase 5`), so the phases 1–5 claims in this report match the commit trail.