...
- **Phase 1 real Foundry mode verification (2026-09-22)** — Azure CLI 2.90.0 authenticated to subscription `Azure for Students`; project access confirmed for `rg-atlas-tutor` / `atlas-personalized-resource` / `atlas-personalized-tutor`; `DefaultAzureCredential` token acquisition succeeded through the local Azure CLI auth path; `python run_agent.py` with Foundry env vars returned a real response from the existing `ATLAS` prompt agent. The adapter still uses `AIProjectClient(..., DefaultAzureCredential(...)).get_openai_client(agent_name=...)`; response encoding is constrained to `gzip, deflate` to avoid a local `httpx2` zstd decoder failure, and `<thinking>/<final>` wrappers are stripped before returning student-facing text. Full backend suite: **83 passed**.
...
- **AI Tutor (§6)** — mock mode fully works; the Foundry prompt-agent chat path (`ask_atlas`) was verified against real Azure on 2026-09-22. The structured turn-analysis and short-answer grading paths still need live output-format verification. Knowledge-base / AI Search wiring is absent.
...
- **Phase 8:** finalized API contract, deployment (PostgreSQL, hosting). Real Foundry prompt-agent chat auth was verified locally on 2026-09-22, but broader production evaluation remains.
...
- Live Foundry smoke test: `python run_agent.py` from `backend/` with `ATLAS_LLM_MODE=foundry`, `AZURE_AI_PROJECT_ENDPOINT`, `AZURE_AI_AGENT_NAME=ATLAS`, and `AZURE_AI_AGENT_VERSION=26` in the process environment or local `.env`. Do not print or commit `.env`.
...
3. **RESOLVED — Foundry prompt-agent chat verified on 2026-09-22.** Official Microsoft Foundry docs confirm `AIProjectClient.get_openai_client(agent_name=...).conversations.create()` / `responses.create(...)`; a live `Hello` smoke test returned a real ATLAS response. Local workaround: send `Accept-Encoding: gzip, deflate` because the installed `httpx2` stack failed on a zstd-compressed response.
